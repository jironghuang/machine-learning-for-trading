...
def get_cash_stocks(order, prices, symbols, start_val):
    ...
    for row in order.itertuples():
        ...
        if leverage > 3.0: #reverse entries made above in df
            df.ix[date:, sym] += - n
            df.ix[date:, 'cash'] += n * sharePrice
...
def compute_portvals(orders_file = "./orders/orders2.csv", start_val = 1000000):
    ...
    cashStocks = get_cash_stocks(order, prices, symbols, start_val)
