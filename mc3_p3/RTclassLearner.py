    ...
    def add_Evidence(self, trainX, trainY):
        ...
        def build_tree(trainX, trainY): # cannot be called from outside this class
            ...
            if trainX.shape[0] <= self.leaf_size: # if no. of train examples < leaf_size, aggregate into leaf
                modeValue, binCount = stats.mode(trainY) # classify: calculate mode & no. of counts of modal value
                return np.array([[-1, modeValue, np.nan, np.nan]])
